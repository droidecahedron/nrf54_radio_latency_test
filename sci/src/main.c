...
#define BT_UUID_VS_SCI_MIN_INTERVAL_SERVICE_VAL BT_UUID_128_ENCODE(0x1c840001, 0x49ac, 0x4905, 0x9702, 0x6e836da4cadd)

#define BT_UUID_VS_SCI_MIN_INTERVAL_SERVICE BT_UUID_DECLARE_128(BT_UUID_VS_SCI_MIN_INTERVAL_SERVICE_VAL)

/** @brief UUID of the SCI Min Interval Characteristic. **/
#define BT_UUID_VS_SCI_MIN_INTERVAL_CHAR_VAL BT_UUID_128_ENCODE(0x1c840002, 0x49ac, 0x4905, 0x9702, 0x6e836da4cadd)
...
static struct bt_le_conn_param *conn_param = BT_LE_CONN_PARAM(INTERVAL_INITIAL, INTERVAL_INITIAL, 0, 400);
...
static ssize_t read_min_interval(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf, uint16_t len,
                                 uint16_t offset)
{
...
BT_GATT_SERVICE_DEFINE(sci_min_interval_svc, BT_GATT_PRIMARY_SERVICE(BT_UUID_VS_SCI_MIN_INTERVAL_SERVICE),
                       BT_GATT_CHARACTERISTIC(BT_UUID_VS_SCI_MIN_INTERVAL_CHAR, BT_GATT_CHRC_READ, BT_GATT_PERM_READ,
...
static uint8_t read_min_interval_cb(struct bt_conn *conn, uint8_t err, struct bt_gatt_read_params *params,
...
static void sci_discovery_complete(struct bt_gatt_dm *dm, void *context)
{
    ...
    if (gatt_chrc)
    {
        ...
        if (gatt_value)
        {
            ...
            LOG_INF("Found SCI min interval characteristic, handle: 0x%04x", remote_min_interval_handle);
        }
        else
        {
...
static void latency_discovery_complete(struct bt_gatt_dm *dm, void *context)
{
    ...
    err = bt_gatt_dm_start(default_conn, BT_UUID_VS_SCI_MIN_INTERVAL_SERVICE, &sci_discovery_cb, NULL);
    if (err)
    {
...
static void connected(struct bt_conn *conn, uint8_t err)
{
    ...
    LOG_INF("Connected as %s", conn_info.role == BT_CONN_ROLE_CENTRAL ? "central" : "peripheral");
    ...
    err = bt_gatt_dm_start(default_conn, BT_UUID_LATENCY, &latency_discovery_cb, &latency_client);
    if (err)
    {
...
void security_changed(struct bt_conn *conn, bt_security_t level, enum bt_security_err err)
{
    ...
    int gatt_err = bt_gatt_dm_start(default_conn, BT_UUID_LATENCY, &latency_discovery_cb, &latency_client);
    if (gatt_err)
    {
...
static int set_conn_rate_defaults(uint32_t interval_min_us, uint32_t interval_max_us)
{
    ...
    LOG_INF("SCI default connection rate parameters set (min=%u us, max=%u us)", interval_min_us, interval_max_us);
    return 0;
}
...
static void conn_rate_changed(struct bt_conn *conn, uint8_t status, const struct bt_conn_le_conn_rate_changed *params)
{
    if (status == BT_HCI_ERR_SUCCESS)
    {
        LOG_INF("Connection rate changed: "
                ...
                params->interval_us, params->subrate_factor, params->peripheral_latency, params->continuation_number,
                params->supervision_timeout_10ms * 10);
    }
    else
    {
        LOG_WRN("Connection rate change failed (HCI status 0x%02x %s)", status, bt_hci_err_to_str(status));
    }
}
...
{
    LOG_INF("LE PHY updated: TX PHY %s, RX PHY %s", phy_to_str(param->tx_phy), phy_to_str(param->rx_phy));
...
static void frame_space_updated(struct bt_conn *conn, const struct bt_conn_le_frame_space_updated *params)
{
    if (params->status == BT_HCI_ERR_SUCCESS)
    {
        LOG_INF("Frame space updated: %u us, PHYs: 0x%02x, spacing types: 0x%04x, "
                "initiator: %s",
                params->frame_space, params->phys, params->spacing_types, fsu_initiator_to_str(params->initiator));
    }
    else
    {
        LOG_WRN("Frame space update failed (HCI status 0x%02x %s)", params->status, bt_hci_err_to_str(params->status));
    }
...
static const struct bt_latency_client_cb latency_client_cb = {.latency_response = latency_response_handler};
    ...
    if (remote_min_interval_handle != 0)
    {
        ...
        if (err == 0)
        {
            ...
            LOG_INF("Minimum connection intervals: Local: %u us, Peer: %u us, "
                    "Common: %u us",
                    local_min_interval_us, remote_min_interval_us, common_min_interval_us);
        }
    }
    ...
    while (default_conn)
    {
        ...
        if (initiate_conn_rate_update)
        {
            ...
            if (current_time >= next_update_time)
            {
                ...
                LOG_INF("Requesting new connection interval: %u us", new_interval_us);
...
#define RADIO_MON_PIN NRF_GPIO_PIN_MAP(1, 11) // 42 P1.11 (connected on p1, unused), NRF_GPIO_PIN_MAP(1, 11)
